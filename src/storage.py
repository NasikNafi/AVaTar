...
class RolloutStorage(object):
    ...
    def compute_returns(self,
        ...
        for step in reversed(range(self.rewards.size(0))):
            delta = self.rewards[step] + gammas * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
            gae = delta + gammas * gae_lambda * self.masks[step + 1] * gae
            avg_gae = gae.mean(-1).unsqueeze(1)
            self.returns[step] = avg_gae + self.value_preds[step]
